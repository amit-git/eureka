...
var stv = require('../../main/resources/public/simple-tree-view');
...
describe('simple tree view', function () {
    describe('verify markup', function () {
        ...
        it('single top level node', function () {
            ...
            var markupBuilder = stv.SimpleTreeView().markupBuilder(testData);
            assert(markupBuilder.build() == ExpectedMarkups.SingleNodeLevelMarkup);
        });
        ...
        it('multiple top level nodes', function () {
            ...
            var markupBuilder = stv.SimpleTreeView({}).markupBuilder(testData);
            assert(markupBuilder.build() == ExpectedMarkups.MultiNodeLevelsMarkup);
        });
        ...
        it('nested node levels', function () {
            ...
            var markupBuilder = stv.SimpleTreeView({}).markupBuilder(testData);
            assert(markupBuilder.build() == ExpectedMarkups.NestedNodeLevelMarkup);
        });
